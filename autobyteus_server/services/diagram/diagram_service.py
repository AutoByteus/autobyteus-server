    ...
    def __init__(self, plantuml_jar_path: Optional[str] = None):
        ...
        Args:
            plantuml_jar_path: Optional path to plantuml.jar. If not provided,
                              will use the path in the download directory.
        """
        # Get download directory from config
        download_dir = app_config_provider.config.get_download_dir()
        
        # Set default plantuml jar path to download directory
        if not plantuml_jar_path:
            plantuml_jar_path = str(download_dir / "plantuml.jar")
        ...
        if not os.path.exists(self.plantuml_jar_path):
            ...
            try:
                ...
                download_url = app_config_provider.config.get("PLANTUML_JAR_DOWNLOAD_URL", self.DEFAULT_DOWNLOAD_URL)
                ...
                error_msg = f"Failed to download PlantUML: {str(e)}. Please place it in the download directory: {download_dir}"
                logger.error(error_msg)
                raise RuntimeError(
                    f"Failed to download PlantUML: {str(e)}. "
                    f"plantuml.jar not found at {self.plantuml_jar_path}. "
                    f"Please place it in the download directory: {download_dir}"
                )
        else:
