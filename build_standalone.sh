...
# Run the Python script to generate dependencies
echo "Running Python script to generate dependencies..."
python copy_dependencies_one_file.py

# New simplified approach: read directly from the file generated by the Python script
DEPENDENCY_ARGS_FILE="nuitka_dependencies.txt"
DEPENDENCY_ARGS_ARRAY=()

if [ -f "$DEPENDENCY_ARGS_FILE" ]; then
  echo "Reading dependency arguments from $DEPENDENCY_ARGS_FILE..."
  while IFS= read -r line; do
    # Skip empty lines
    if [ -n "$line" ]; then
      DEPENDENCY_ARGS_ARRAY+=("$line")
    fi
  done < "$DEPENDENCY_ARGS_FILE"
  echo "Successfully read dependency arguments from file."
else
  echo "Warning: Dependency arguments file not found: $DEPENDENCY_ARGS_FILE"
fi
